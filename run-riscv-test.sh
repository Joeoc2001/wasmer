...
RUSTFLAGS="-C linker=riscv64-linux-gnu-gcc -C target-feature=+crt-static" cargo build --target=riscv64gc-unknown-linux-gnu --example riscv --release --no-default-features --features "singlepass" --config profile.release.debug=true
